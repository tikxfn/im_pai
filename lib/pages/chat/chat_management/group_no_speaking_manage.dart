...
class GroupNoSpeaking extends StatefulWidget {
  const GroupNoSpeaking({super.key});

  static const String path = 'friends/group_no_speaking_update';

  @override
  State<StatefulWidget> createState() {
    return _GroupNoSpeakingState();
  }
}

class _GroupNoSpeakingState extends State<GroupNoSpeaking> {
  String keywords = '';
  String roomId = ''; //群id
  double size = 44; //头像大小
  int limit = 50;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text('禁言成员()'.tr(args: [userList.length.toString()])),
      ),
      body: KeyboardBlur(
        child: ThemeBody(
          child: Column(
                      ...
                      children: [
                        ...
                        CircularButton(
                          onTap: () {
                            Navigator.pushNamed(
                              context,
                              GroupNoSpeakingUpdate.path,
                              arguments: {
                                'roomId': roomId,
                                'isenableProhibition': true,
                              },
                            ).then((value) {
                              _getList(init: true);
                            });
