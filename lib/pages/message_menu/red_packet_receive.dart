...
class RedPacketReceivePage extends StatefulWidget {
  const RedPacketReceivePage({super.key});
  static const String path = 'redPacketReceive/page';

  @override
  State<RedPacketReceivePage> createState() => _RedPacketReceivePageState();
}

class _RedPacketReceivePageState extends State<RedPacketReceivePage> {
  //是否是收到的红包
  bool _isReceive = true;

  List<GRedEnvelopeReceivedModel> _receiveList = [];
  List<GRedEnvelopeSendModel> _sendList = [];
    ...
    return Column(
      children: [
        ListTile(
          ...
          trailing: Text('${api2number(model.amount)} 元'),
        ),
        Padding(
  ...
  void _showCupertinoDialog(BuildContext context) {
    var myColors = context.read<ThemeNotifier>();
  ...
  _requestDataReceive() async {
    try {
      ...
      var res = await RedEnvelopeApi(apiClient()).redEnvelopeReceivedList(
          V1RedEnvelopeReceivedListArgs(pager: GPagination(limit: '100')));
      if (res != null) {
        // tipSuccess('数据获取成功');
  ...
  _requestDataSend() async {
    try {
      ...
      var res = await RedEnvelopeApi(apiClient())
          .redEnvelopeSendList(GPagination(limit: '100'));
      if (res != null) {
        // tipSuccess('数据获取成功');
        _sendList = res.list;
        setState(() {});
